...
public class ModularCartHandler extends SyncedGuiDescription {
	public ModularCartHandler(int syncId, PlayerInventory playerInventory, ModularMinecartEntity minecartEntity) {
		...
		WListPanel<Configurable, WPlainPanel> panelList = new WListPanel<>(minecartEntity.getModuleList().stream().filter(Configurable.class::isInstance).map(Configurable.class::cast).toList(), WPlainPanel::new, Configurable::configure);
		panelList.setSize(200, 256);
