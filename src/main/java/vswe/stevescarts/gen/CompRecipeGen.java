...
public class CompRecipeGen {
	...
	private static void concern() throws Throwable {
		Path componentRecipePath = Paths.get(".").resolve("..").resolve("src_old").resolve("src").resolve("main").resolve("resources").resolve("assets").resolve("stevescarts").resolve("recipes").resolve("component");
		Path finalRecipePath = Paths.get(".").resolve("..").resolve("src/main/resources/data/stevescarts/recipes/component");
		for (Path recipePath : Files.walk(componentRecipePath).collect(Collectors.toSet())) {
			if (Files.exists(finalRecipePath.resolve(recipePath.getFileName()))) {
				continue;
			}
			try {
				JsonObject json = GSON.fromJson(Files.newBufferedReader(recipePath), JsonObject.class);
				JsonObject key = json.getAsJsonObject("key");
				for (String s : key.keySet()) {
					conc(key.getAsJsonObject(s));
				}
				conc(json.getAsJsonObject("result"));
				Path newPath = finalRecipePath.resolve(recipePath.getFileName());
				Files.createDirectories(newPath.getParent());
				Files.write(newPath, GSON.toJson(json).getBytes());
			} catch (Throwable e) {
				System.err.println("Error processing " + recipePath);
			}
		}
	}

	private static void conc(JsonObject object) {
		if (object.has("item") && object.has("data")) {
			if (object.get("item").getAsString().equals("stevescarts:modulecomponents")) {
				int data = object.get("data").getAsInt();
				String newType = ComponentTypes.values()[data].lowerName();
				object.remove("data");
				object.remove("item");
				object.addProperty("item", "stevescarts:%s".formatted(newType));
			}
		}
	}
	...
	public enum ComponentTypes {
		...
		TANK_VALVE(60, "SCTank Valve"),
		TANK_PANE(61, "SCTank Pane"),
		LARGE_TANK_PANE(62, "Large SCTank Pane"),
		HUGE_TANK_PANE(63, "Huge SCTank Pane"),
